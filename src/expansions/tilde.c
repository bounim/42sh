...
int     tilde_result(char **env, t_word *tmp, size_t start, size_t tilde_prefix_len)
{
...
int		do_tilde_assign(char **env, t_word *assign_head)
{
	t_word	*tmp;
	size_t	len;
	size_t	i;
...
int     tilde_expansion(char **env, t_parser_node **head)
{
	...
	do_tilde_assign(env, (*head)->assign_head);
